...
@interface PhotosEditViewController ()<UITableViewDelegate,UITableViewDataSource,UIImagePickerControllerDelegate,UINavigationControllerDelegate,SencondAddPhotosViewControllerDelegate>{
...
-(void)delPhotosBtnAction{
    ...
    UIAlertAction *sureAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
        NSMutableDictionary *parameter = [NSMutableDictionary dictionary];
        parameter[@"token"] = [UserInfo getToken];
        parameter[@"act"] = @"del_album";
        parameter[@"album_id"] = _albumId;
        [self showProgressHud:MBProgressHUDModeIndeterminate message:nil];
        [ZMCHttpTool postWithUrl:WLMinePhotosURL parameters:parameter success:^(id responseObject) {
            NSLog(@"%@", responseObject);
            if ([responseObject[@"status"] isEqualToString:@"1"]){
                [self hideProgressHud];
                NSLog(@"%@", [[self presentingViewController] class]);
                RootViewController *rootVC = (RootViewController *)[self presentingViewController];
                NSLog(@"%@", [rootVC.viewControllers[3] class]);
                WLNavigationController *wlNav = rootVC.viewControllers[3];
                [wlNav popViewControllerAnimated:YES];                
                [self dismissViewControllerAnimated:YES completion:nil];
            }
            else {
...
-(UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    if (section==1){
        ...
        UIButton *delPhotosBtn = [[UIButton alloc]initWithFrame:CGRectMake(10, 10, screen_width-20, 50)];
        [delPhotosBtn setTitle:@"删除相册" forState:normal];
        [delPhotosBtn addTarget:self action:@selector(delPhotosBtnAction) forControlEvents:UIControlEventTouchUpInside];
        delPhotosBtn.backgroundColor = ThemeColor;
        [delPhotosBtn setTitleColor:[UIColor whiteColor] forState:normal];
        [footerView addSubview:delPhotosBtn];
        return footerView ;
    }
    return nil;
}
